                      ...
                      cartItems.map((item) => (
                        <tr className="product-row">
                          <td className="product-item-img">
                            <img src={item.productimage} alt="img" /></td>
                          <td className="product-item-name">{item.pname}</td>
                          <td className="product-item-price">${(item.pprice).toFixed(2)}</td>
                          <td className="product-item-totle">{item.cartQuantity}</td>
                          <td className="product-item-close text-end">
                            <a className="ti-close cursor-pointer" onClick={() => dispatch(removeFromCart(item))}><IoCloseSharp /></a></td>
                        </tr>
                      ))
                    }
                    <tr>
                      <td colSpan="5"> {
                        cartItems.length === 0 && <h5 className='text-center m-auto'>Cart is Empty<Link to={'/shop'}><em> Shop Now</em></Link></h5>
                      }</td>
                    </tr>
                    ...
                    <div className='col-md-4' key={idx}>
                      <div className='spritual-book-inner-prt'>
                        <Link to={`/products/${data.id}`}>
                          ...
                          <p className='mt-0 text-truncate text-info'>{data.author}</p></Link>
                      </div>
                    </div>
                  )
                })
